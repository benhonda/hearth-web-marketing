...
- [ ] **2026-04-21 · Real page & cover layouts (deferred from design binder, Flow II)**
  ...
  **Context.** While drawing the `02-make-a-book` flow in the design binder, we reached Screen VI (Global settings), where the user picks a cover layout and one page layout per photo-count bucket (1, 2, 3+). The thumbnails in that picker had been abstract skeleton sketches — solid rectangles and hairlines. Ben flagged that this would force us to keep two sources of truth: the skeleton thumbnails here, and the real rendered layouts on the spread-preview / per-spread-edit screens later. We agreed the right pattern is **one component rendered at two scales** — full-size on VIII/IX, scale(0.42) inside the VI picker frames.
  ...
  **What currently exists (skeleton, not final):**
  ...
  - `BookSettings.astro` wires these in via `transform: scale(0.42)` inside a fixed 4:5 frame with `overflow: hidden`.
  ...
  **What's explicitly deferred — this is the "circle back" work:**
  ...
  5. **Spread semantics.** VIII is "spread preview" — two facing pages side-by-side. A recipe might be a one-pager on a single page (facing another recipe or a chapter mark), or a two-page spread. The current `BookPage` component is ONE page; we'll need a `BookSpread` wrapper that composes two `BookPage`s (or one full-bleed spread that spans both halves, for the 1-photo-b layout).
  ...
  7. **Per-spread layout override UI.** Screen IX (Edit a spread) is the moment a user overrides the global default for a specific spread's photo count. The override control itself (a small layout-swap above the spread) needs design.
  8. **Cover-photo coupling.** When a user picks a cover photo (screen VII) from the selected recipes, we need to track WHICH recipe that photo came from — for photo credit on the colophon, and to ensure the cover photo isn't also used unmodified in the book's interior.
  ...
  **Not lost work.** The `BookPage` / `BookCover` component contracts (props, slot semantics, natural size) are the right shape; only the *internals* of each layout need the real design pass. The wiring into `BookSettings.astro` (scale transform + frame pattern) is the right pattern and will remain when we upgrade the layouts.
