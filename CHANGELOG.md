...
- feat: Added Flow V (`/design/flows/05-friends`) — *Friends.* Exploratory, beyond V1 scope: visualizes the social-layer brainstorm so the brand voice can be tested against the idea before committing. Five screens — Library with options doorway → Friends directory (Sarah Chen et al., italic Lora names + collection-titles-as-byline, no avatars) → Sarah's shared collection (Apple-Music-playlist analog with FROM SARAH CHEN credit byline) → Recipe with credit chain (FROM table.kitchen · VIA Sarah Chen attribution row, Save grammar) → Library after friend-save (waterfall quietly grown by one tile, "via Sarah Chen" as a third caption line on the new recipe). Establishes collections-as-social-atom, library-stays-private, bilateral-mutual-graph framing.

- feat: Added Measures sub-flow under Flow II — three new screens (`recipe-detail-units`, `recipe-detail-units-picker`, `recipe-detail-in-grams`) and four new `RecipeDetail` props (`units`, `unitsPickerOpen`, `unitsConverted`, plus `recipe` override). The Ingredients eyebrow row now carries IN CUPS/GRAMS/OZ on the right, twin of "serves N" on the left, in the same DM Sans 500 small-caps Ochre + 0.75px ochre underline grammar. The picker blooms in place into a typographic radio strip — `— cups · grams · oz —` — sharing portion-scale's vocabulary at lower volume (no overlay, no popover; the eyebrow row IS the picker). New `methodInGrams` / `ingredientsInGrams` static fixtures on `cornRagu` so the conversion lens shows real outcomes without runtime math.

- refactor: Flow I import restructured around a textarea (not a single-line URL field) that accepts whatever the cook hands it — link, recipe texted from a friend, chunk pasted from Notes — with AI auto-detecting on submit. Below the textarea, a co-equal "Scan a photo" doorway with em-dash "or" divider; both options visible at once, the cook picks by acting. The photo doorway opens a new Photo sub-flow off Import · surface — iOS source sheet → **transcription gate** (`import-transcription`: verbatim OCR with dotted-Ochre underlines on low-confidence words, the cook reviews against the original photo before structuring) → photo-variant Import preview (`import-preview-photo`: paper-warmth artifact hero standing in for the cook's photograph, "from your photograph" caption, Notes block pre-populated with AI-extracted margin text). Separates two failure modes — transcription errors handled at the gate, structure errors at the preview — so heirloom handwritten cards have an honest place to be reviewed. The Safari share-sheet path remains as the alternate sub-flow.

- refactor: `ImportPreview` rewritten as the **structured edit** — the shared destination for all three import doorways (URL / paste / photo). Every field is inline-editable with quiet, earned affordances: title in Lora display (no form-field box), Serves N + minutes carry dotted-Ochre underlines, ingredients render as a type-aware trio per line — qty (dotted underline) · unit (dotted underline + ▾ — combobox grammar accepting standard units AND "pinch", "small handful", "to taste") · name. Notes block sits between meta and ingredients (empty as invitation on URL/paste; populated on photo). AI-extracted inline comments arrive pre-rendered where the source carried marginalia. New `source: "url" | "photo" | "text"` prop drives the hero, from-card, and recipe content variants.

- refactor: `Library.astro` consolidated from three states to two — the `book-in-progress` state was removed entirely. The `populated` state now always carries an options list above the waterfall (Collections · Friends · Cookbooks), and the book-creation entry point moved off the foot ceremony into the Cookbooks row. Every top-level destination — books, collections, friends — now lives in one nav grammar, sibling rows of the implicit "all recipes" view below. The `LibraryBookInProgress` component, its render key, and its detail page were deleted. New `optionsTapTarget` + `tilesOverride` props let Flow III/IV/V drive halos on specific options rows and seed credit-chain tiles into the waterfall.

- refactor: Make-a-book Layout step promoted from the Settings sub-flow back onto the main line as step V (between Recipes and Preview). The cook now picks cover + per-bucket page layouts BEFORE seeing the book — the boutique route's hand, not a template the system picks. Layout still lives in Settings for revisiting after Preview. Flow III's main line is now ten numbered steps (was nine).

- feat: `FlowStep.branch` (singular) replaced with `branches: FlowBranch[]` (plural) so a single main-line step can spawn multiple sub-flows. `FlowStoryboard` re-grids to stack branches vertically beneath their parent step — branch 0 at rows 2/3, branch 1 at rows 4/5, etc. — keeping every sub-flow inside the same horizontal scroller. Powers Flow II's five sub-flows under one springboard step and Flow I · III's two doorways (Photo + Share-sheet).

- feat: Added `DesignSidebar` — a sticky left TOC for the `/design` index that lists every flow + its named sub-flows, derived entirely off `flows.ts` (no manual config). IntersectionObserver-driven active highlight tracks the user's scroll position through the binder. The full binder is now jumpable from the top instead of requiring a long horizontal scan.

- refactor: `sample-recipe.ts` reshaped from a single `sampleRecipe` singleton into a `Recipe[]` registry with required core fields (slug, title, source, url, serves, minutes, lede, ingredients, method) and optional rich extras (`editedMethod`, `methodDoubled`, `methodInGrams`, `photo`, `photos`, `tone`, `tileAspect`). Stub recipes that omit `photo` fall back to a tone-gradient hero so a new recipe renders end-to-end without new image assets. The corn-ragù-specific story exports stay separate (they're scene state for one arc, not data any other recipe could carry).

- feat: Added `/design/screens/recipes/[slug]` — a prerendered dynamic route that generates a static detail page per entry in the `recipes` registry. Library waterfall tiles now link to working detail pages instead of dead-ending; appending a `Recipe` object to the registry automatically populates the head of the waterfall, gives the tile a working `href`, and creates the leaf detail page — no per-recipe boilerplate. Added `carrot-cake-hero.jpg` / `carrot-cake-slice.jpg` under `web/public/design/library/` as the second registered recipe.

- refactor: `RecipeDetail` gained a `recipe?: Recipe` prop so any registry recipe can drive the canonical detail page (defaults to `cornRagu`); per-step variant fixtures (`editedMethod`, `methodDoubled`, `methodInGrams`) all fall back to base `method` when a stub recipe doesn't define them — the screen still renders, it just doesn't show the variant content. New `heroType: "photo" | "carousel"` + `heroPhotos[]` props let the same detail component render both single-photo and multi-photo recipes off one source.

- refactor: `screenComponents` registry typed as `Partial<Record<ScreenRender, any>>` so a render key can be added to the union before its component lands; missing entries render as "Next pass" placeholders, which is the desired in-progress-draft behavior. Removes the type-error friction that previously forced every render-key addition to ship with its component in the same change.

- docs: PRODUCT.md gained an 8th load-bearing principle — *Credit travels with the recipe, not the user* — and the social-layer framing throughout: Friends as bilateral mutuals, Collections as the unit of sharing (Apple-Music-playlist analog), credit chain (FROM origin · VIA curator) preserved on the printed page like real cookbook attributions ("submitted by Mrs. Thompson, originally her grandmother's"). Added "Not a feed." as a load-bearing anti-statement — no follower counts, no activity stream, no public profiles, no Wrapped, no FOMO. Apple-Music-shape, never Spotify-shape.

- docs: README restructured. Priorities collapsed into one ordered top-to-bottom list (Drawn · To draw next · V1-scoped lower priority · Load-bearing rules) replacing the prior split phases. Added Friends as Drawn (Flow V). Added "How this started" — the carrot-cake origin story behind the product. Recipe import rewritten as three co-equal doorways (link · paste text · photo) with the transcription-gate architecture. Load-bearing rules expanded with credit-chain rendering, library-stays-private, bilateral-mutual-graph framing, notes-never-travel. Shelved section converted from a table to a bullet list (more breathing room for the now-longer triggers). "Cover options beyond V1" moved from the Deferred-book-layout note's item 6 into Shelved (it's a *don't-build-at-V1* decision, not unfinished V1 work). To-draw-next gained Your collections, Friend invite/accept, and Friends-list edge states.

- docs: `hearth-binder` skill (SKILL.md) updated — README pointer rewritten to reflect the consolidated single-list priorities + bullet-list Shelved structure, flow-step count updated (12 was the old Flow I; the binder now reads as five flows with sub-flows), folio convention example updated from `— page 1 · version ii —` to `— page 1 —` matching yesterday's version-ornamentation removal.

- chore: CLAUDE.md gained a "KISS" shorthand — when Ben says "KISS" it's a gentle reminder to *keep it simple, stupid* — so future sessions decode the abbreviation without round-tripping.

## 2026-04-25

- refactor: Flow I restructured to put paste-a-URL **inside Hearth** as the primary import path. The old Library-empty → Mobile Safari → Share sheet → Import preview main line is replaced with Library-empty → Import surface → Import surface (pasted) → Import preview; the Safari + share-sheet sequence is demoted to a sub-flow that branches off the new Import surface step. Same Import preview lands the cook regardless of doorway. Two new screens — `import-surface` (empty paste field, halo on input) and `import-surface-pasted` (URL filled in, full-width Foil-Light Primary "Import" pill below — same grammar as Flow II's "Send to Hearth Press"). Demonstrates that web URL import is a first-class action without leaving the app, matching the README's "Web URL import only" V1 rule.

- refactor: `Library.astro` extracted as the single source of truth for the library home, rendering one of three states via a `state` prop — `empty` / `populated` / `book-in-progress`. The existing `LibraryEmpty`, `LibraryPopulated`, `LibraryBookInProgress` files are now ~10-line wrappers passing the state in. One file owns wordmark spacing, count rhythm, invitation grammar (italic-Ochre minimal line for empty; Hearth Press eyebrow + chapter rule + italic line + Ochre CTA for populated/in-progress), and halo geometry — no more parallel-edit drift across the three.

- refactor: Library-empty foot invitation switched from external Safari handoff to an in-app italic-Ochre "— import a recipe →" line that opens the new Import surface. Also dropped the prior teach paragraph ("Share from Safari, TikTok or Instagram to set the first one.") — caption import is Shelved in the README and the chapter-opener already carries the editorial weight.

- refactor: `RecipeSaved` cleaned up — dropped the "edited just now" italic-Ochre meta marker AND the "Compare with the original" tertiary link at the foot. Compare-to-original is Shelved (yesterday's scope call); the saved screen is now visually identical to the canonical detail page except for step iv's wording, which is the entire point of the book-spine test.

- refactor: Folio "· version iii" ornamentation removed from `RecipeDetailCarousel`, `BookPageEditPickHero`, and the tokens.css folio example — folios now read just "— page 1 —". Versioning was visual ceremony for a model (recipe versions) we don't ship; removing it lets the page change quietly, matching the rest of the book-spine grammar.

- refactor: `RecipeEditMenu` and `RecipeDetail` gained pass-through props for the annotation layer (`haloTarget`, `methodComments`, `ingredientComments`, `ingredientTapIndex`, `folio`, `slot="notes"`) so Flow IV/V can drive notes and per-line comments through the same canonical detail/menu components rather than forking — yesterday's Flow IV/V changelog described the surfaces; today completes the pass-through wiring.

## 2026-04-24

- feat: Added Flow IV (`/design/flows/04-notes`) — the notes arc. A single Apple-Notes-style free-text block per recipe lives as a peer section to Ingredients and Method, between meta and the first rule — reading order becomes author's voice → cook's voice → what you need → how to make it. Five screens cover the empty-to-first-write-to-revise arc: `recipe-detail-empty-note` (invitation halo'd), `recipe-note-compose` (first write), `recipe-detail-with-note` (populated block, tap-to-edit halo), `recipe-note-edit` (revision), `recipe-detail-expanded-note` (both voices flowing in one block — cook-note + bracketed-em-dash memory-note). New `RecipeNotes` atom renders the block across all states; `RecipeDetail` gained a `slot="notes"` + optional folio prop.

- feat: Added Flow V (`/design/flows/05-line-comments`) — line-item comments. Mirrors Flow IV's grammar at finer scope: the cook annotates a specific method step or ingredient, and the comment renders beneath it in a smaller italic register. Six screens: `recipe-detail-pre-comment` (carries Flow IV's end-state), `recipe-step-comment-menu` (Comment-primary long-press menu), `recipe-step-comment-compose`, `recipe-detail-step-commented` (halo moves to an ingredient line), `recipe-ingredient-comment-compose` (menu screen skipped — same grammar as II), `recipe-detail-fully-annotated` (the "Megan's version" payoff). New `ItemComment` atom + pass-through props on `IngredientsList` / `MethodList` / `RecipeDetail` for per-item annotation.

- refactor: `RecipeEditMenu` gained a `Comment` row above Edit/Delete and a `haloTarget: "comment" | "edit"` prop. One component, one menu, halo is the storyboard overlay. Flow I · X continues to halo Edit (default); Flow V · II halos Comment. The Comment-primary order reflects the 90% case (annotate, don't rewrite) while Edit keeps its row for the rare fork.

- docs: PRODUCT.md gained a 7th load-bearing principle — *Annotations are the personalization layer* — and the job-to-be-done line now reads "Save every recipe I love, make it mine as I cook, and — when the moment calls for it — turn it into a real book." The personalization framing is the strongest North Star Hearth has: the printed book carries the cook's annotations alongside the recipe body, making a library of somebody-else's recipes visibly hers.

- docs: README V1 scope §4 split into §4 (Annotations) + §5 (Recipe edit — rare fork), renumbering the rest through §10. Build order step 6 rewritten as "Annotations + edit + unit conversion — the craft sprint." Deferred book-layout work gained item 9 — marginalia rendering — flagged as load-bearing and cross-cutting with items 1–3 when the real layout pass happens.

## 2026-04-23

- docs: Consolidated `MVP.md` and `/design/roadmap` into a single scope section inside `README.md`, then deleted both. V1 scope, Shelved table (with triggers), build order, open follow-ups, and the deferred book-layout note now live in the README — one source of truth, no parallel docs to drift. Scope call baked in alongside the consolidation: TikTok/IG caption import + Compare-to-original demoted from V1 into Shelved (off-wedge; web URL import covers the MVP path); scaling + unit conversion promoted from Shelved into V1 §5 (scaling is already drawn end-to-end in Flow I; grams-not-cups is the next-up craft-sprint work alongside notes / edit-beyond-line-items). Removed the `Roadmap` entry from the `/design/*` topbar, pointed the `hearth-binder` skill's source-of-truth list at `README.md`, and replaced the `MVP.md` reference with `README.md` in the third slot.
